...
if (!isset($_SESSION['user_id'])) // If the user is unauthenticated / logged out, redirect them to the login page
{
		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-lg-3 col-md-4 col-sm-5" style="background-color: #f1f1f1; height: 100vh; border-right:1px solid #ccc;">
					...
					?>
							...
							<div class="mt-3 mb-3 text-center">
								...
								<br> -->
								<img src="<?php echo $_SESSION['user_profile']; ?>" class="img-fluid rounded-circle img-thumbnail" width="150" />
								<h3 class="mt-2"><?php echo $_SESSION['user_name']; ?></h3>
							...
							<div class="search-container">
								<input type="text" id="search" name = "search" placeholder="Search..." oninput="filterUser()" required>
								<button type="button" for = "search" onclick="">
									<img src="https://img.icons8.com/material-outlined/24/4CAF50/search.png"  alt="Search Icon">
								</button>
							</div>
					<?php
						// }
					...
					<?php
						...
						FROM users ORDER BY login_status desc"
						;
					...
					<div class="list-group" style=" max-height: 60vh; margin-bottom: 10px; overflow-y:scroll; -webkit-overflow-scrolling: touch;">
						<table id = "usertable">
						<?php
							foreach ($user_data as $key => $user)
							{
								...
								if ($user['id'] != $_SESSION['user_id']) // Display/Show ONLY the Unauthenticated users i.e. Show all users EXCEPT the authenticated user
								{
													...
													<a class='list-group-item list-group-item-action select_user' style='cursor:pointer' data-userid = '" . $user['id'] . "'>
														...
														<span class='ml-1'>
															<strong>
																...
																<span id='userid_" . $user['id'] . "'>" . $total_unread_message . "</span> <!-- Show the 'Unread' messages count red-colored Push Notification -->
															</strong>
														</span>
				...
				<div class="col-lg-9 col-md-8 col-sm-7">
					<br />
					<h3 class="text-center">Chatting Room</h3>
					<hr />
					<br />
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			filterUser = function() {
				...
				filters = filterInput.value.toLowerCase();
				...
				rows.forEach(row => {
					...
					cells.forEach((cell, index) => {
						if (filters && !cell.textContent.toLowerCase().includes(filters)) {
							visible = false;
						}
					});
			...
			var conn = new WebSocket('ws://<?php echo SERVER_IP?>?token=<?php echo $token; ?>'); // Create the Browser Web API WebSocket object i.e. Start the WebSocket connection!    // Initiate/Start the WebSocket connection in the browser. Check the browser's console.
				...
				// if (data.command == 'private') // To avoid the problem that the Group Chat messages of some user to show up in their (he/she) One-to-One/Private Chat Area on another user's account
				{
					...
					if (receiver_userid == data.userId || data.from == 'Me') // If there's a received message for the authenticated/logged-in user throug WebSocket (mostly sent from the onMessage() method of Chat.php Class) (because we're already inside    conn.onmessage    event) and while so, if the autheticated/logged-in user is already having a user's private chat area clicked/opended and that specific user has sent that authenticated/logged-in user a message (which all means the authenticated/logged-in is ONLINE/CONNECTED), OR, the authenticated/logged-in user themselves (he/she) has sent a message to some user (which also means that the authenticated/logged-in user is ONLINE/CONNECTED)
					{
						if ($('#is_active_chat').val() == 'Yes') // If the Private Chat Area (with any specific user) is opened
						{
							var subtag = '';
							if(data.type){
								subtag += `<a href = "uploads/${data.msg}">${data.msg}</a>`
							}else{
								subtag += data.msg;
							}
							var html_data = `
							...
							// Empty the chat <textarea> after the chat message has been sent (in case the authenticated/logged-in user sent that message or they (he/she) received a message from another user)
						}
					}
			...
			function make_chat_area(user_name) // When a user is clicked on to chat with (on the left side in the Private Chat)    // user_name    function parameter is the user name of the clicked user (receiver)
			{
				var html = `
					...
					<form id="chat_form" method="POST" data-parsley-errors-container="#validation_error" enctype="multipart/form-data">
						<div class="input-group mb-3" style="height:7vh">
							...
							<div class="input-group-append">
								<div class = "file-upload-container" id = "file-div">
									
									
									<input type="file" name = "file" id="fileInput" hidden/>

									<label for="fileInput" style = "margin-bottom:0rem;cursor:pointer;"><i class="fa fa-paperclip"></i></label>
									<span id="file-name" style = "display:none"></span>
									<span id="progress" style="margin-left:1rem; display: none;"><span id="progressValue">0</span>%</span>
								</div>
					...
					success:function(data) // 'data' is the response (Chat History between the two users) from the server/server side/backend (action.php)
					{
						if (data.length > 0) // If there's a chat history between the authenticated/logged-in user and the selected user
						{
							...
							for (var count = 0; count < data.length; count++)
							{
								...
								if(type){
									subtag += `<a href = "uploads/${data[count].chat_message}">${data[count].chat_message}</a>`
								}else{
									subtag += data[count].chat_message;
								}
			...
			$(document).on('submit', '#chat_form', function(event){
				...
				if(filename.text() != ""){
					xhr.open('POST', 'msgupload.php', true); // Open the connection

					// Update progress
					xhr.upload.addEventListener('progress', function(event) {
						if (event.lengthComputable) {
							var percentComplete = (event.loaded / event.total) * 100;
							$('#progress').show(); // Show the progress span
							$('#progressValue').text(Math.round(percentComplete)); // Update the progress value
						}
					});
					...
					xhr.onload = function() {
						if (xhr.status === 200) {
							...
							console.log(xhr.responseText); // Display the response from PHP
							var user_id = parseInt($('#login_user_id').val()); // The authenticated/logged-in user's `user_id` in `chat_application_users` table
							var message = filename.text(); // The submitted Private Chat message
						...
						$('#progress').hide(); // Hide the progress span after upload
					};
			...
			$(document).on('change', '#fileInput', function() {
				var filename = $('#file-name');
				// console.log(this.files.length); // This should only log once per click
				if(this.files.length > 0){
					filename.attr({"style":"display:inline-block"});
				...
				$.ajax({
					...
					success:function(data) // 'data' is the response from the server (server-side/backend). It contains the 'status' key. Check the first if condition in action.php
					{
						...
						if (response.status == 1) // 'data' is the response from the server (server-side/backend). It contains the 'status' key. Check the first if condition in action.php
						{
							...
							location = 'dashboard.php'; // Redirect the user to the Login Page (index.php) after logging out
						}
					}
				})
			});
